                        ...
                        const zone_node_t *node)
{
	...
	if (node->rrset_count > 1 && !only_dnssec(node)) {
		...
		log_semantic_error(zone_name, node, CNAME_EXTRA_RECORDS);
		return false;
	}

	if (cname_rrs->rr_count > 1) {
		log_semantic_error(zone_name, node, CNAME_MULTIPLE);
		return false;
	}
